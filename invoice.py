...
def get_random_client():
    ...
    client = Client(
        name=person.get("company"), address1=street, address2=address, nip=fake.itin()
    )
    return client
...
def generate_invoices(number):
    for i in range(number):
        ...
        invoice = Invoice(
            get_random_client(),
            get_random_provider(),
            fake.pyint(min_value=10000000, max_value=99999999),
            start_date,
            end_date,
            fake.city(),
        )
        ...
        with open(
            PATH_TO_DATA_SOURCES + category + ".csv", newline="", encoding="utf8"
        ) as csvfile:
            ...
            SimpleInvoice(
                invoice,
                PATH_TO_GENERATED_INVOICES + "invoice_",
                "en",
                faker=fake,
